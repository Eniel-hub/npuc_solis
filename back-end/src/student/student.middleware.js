...
const GetReligion = async (req, res, next) => {
  //get a religion based on id
  const id = req.params.id;
  try {
    const response = await service.GetReligion(id);
    if (!response) res.json({ error: "no religion found" });
...
const GetNation = async (req, res, next) => {
  ...
  try {
    ...
    if (!response) res.json({ error: "no nation found" });
    else res.json(response);
  } catch (err) {
...
const studentExistInSchool = async (req, res, next) => {
  let user = await userMiddleware.GetUser({ ID: req.body.studentID });
  if (user)
    return res.json({
      isFound: false,
      message: "ID Number link to an accound. please login first.",
    });
  let student = await service.GetStudent(req.body.studentID);
  if (!student)
    return res.json({
      isFound: false,
      message: "no student found with the entered ID Number",
    });
  if (student.school_id == req.body.schoolId)
    return res.json({ isFound: true });
  return res.json({
    isFound: false,
    message: "this ID Number doesn'e belong to the school selected",
  });
};

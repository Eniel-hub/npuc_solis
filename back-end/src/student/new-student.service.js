...
const RegisterNewStudent = async (username, student) => {
  student.fullname = helper.FullName(
    student.firstname,
    student.lastname,
    student.middlename
  );
  student.father.pname = student.father.firstname
    ? helper.FullName(
        student.father.firstname,
        student.father.lastname,
        student.father.middlename
      )
    : "";
  student.mother.pname = student.mother.firstname
    ? helper.FullName(
        student.mother.firstname,
        student.mother.lastname,
        student.mother.middlename
      )
    : "";
  student.guardian.pname = student.guardian.firstname
    ? helper.FullName(
  ...
  try {
    await UpdateLoginInfo(username, studentId);
  } catch (err) {
    console.log(err.message);
  }
...
const SaveNationality = async (nationality) => {
  await db.Query("INSERT INTO student_nationality (nationality) VALUES (?)", [
    nationality,
  ]);
...
const GetNationalityId = async (nationality) => {
  const result = await db.Query(
    "SELECT (ID) FROM student_nationality WHERE nationality = ?",
    [nationality]
  );
  return result[0].ID;
};
...
const SaveStudent = async (student) => {
  await db.Query(
    `INSERT INTO student (lastname, firstname, middlename, fullname, 
...
const GetStudentId = async (student) => {
  ...
  result = await db.Query("SELECT id FROM student WHERE fullname = ?", [
    fullname,
  ]);
...
const getGuardian = (student) => {
  if (student.Guardian == "Father") return student.father.pname;
  if (student.Guardian == "Mother") return student.mother.pname;
  if (student.Guardian == "Guardian") return student.guardian.pname;
...
const UpdateLoginInfo = async (username, id) => {
  await db.Query("UPDATE student_login SET student_id = ? WHERE username = ?", [
    id,
    username,
  ]);
};
